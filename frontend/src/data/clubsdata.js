export default {
    orgs: [
        {
            name: 'CALL OF DUTY',
            picture: '/Pictures/Teams/CODColdWar.png',
            pictures: [
                '/Pictures/Teams/CODColdWar.png',
                '/Pictures/Teams/overwatch.jpg',
            ],
            socials: [
                {
                    type: 'discord',
                    link: 'https://discord.gg/ERUvB6Z',
                },
            ],
            logo: '/Pictures/Logos/COD.png',
            people: [
                {
                    name: 'Victor Okoye',
                    role: 'PRESIDENT',
                },
                {
                    name: 'Jeff Zhang',
                    role: 'VICE PRESIDENT',
                },
                {
                    name: 'Alexander Li',
                    role: 'TREASURER',
                },
                {
                    name: 'Kevin Su',
                    role: 'COMPETITIVE MANAGER',
                },
            ],
            meeting: 'Every other Friday 7:30 PM - 8:30 PM',
            location: 'On our Discord!',
            about: 'Hello there! We’re Rutgers Women’s Gaming League. We’re here to promote equality in gaming and provide a safe and quality environment for gamers of all kinds while uplifting women. We hope to provide the women of Rutgers a comfortable place to express their passion for gaming who are seeking a community where they feel they belong. We value creating a club where everyone feels like they have a space and have people to share their hobbies with, and support all genders in participating in competitive and non competitive gaming. If you’re looking for like-minded friends or just a place to hang out, come check us out! We also hold events and tournaments throughout the semester, be on the lookout for those!',

        },
        {
            name: 'CS:GO',
            picture: '/Pictures/Teams/csgo.png',
            pictures: [
                '/Pictures/Teams/csgo.png',
                '/Pictures/Teams/lol.png',
            ],
            socials: [
                {
                    type: 'facebook',
                    link: 'https://www.facebook.com/groups/682133058538104/',
                },
                {
                    type: 'discord',
                    link: 'https://discord.gg/pUzpQXY',
                },
            ],
            logo: '/Pictures/Logos/csgoicon.png',
            people: [
                {
                    name: 'Victor Okoye',
                    role: 'PRESIDENT',
                },
                {
                    name: 'Jeff Zhang',
                    role: 'VICE PRESIDENT',
                },
                {
                    name: 'Alexander Li',
                    role: 'TREASURER',
                },
                {
                    name: 'Kevin Su',
                    role: 'COMPETITIVE MANAGER',
                },
            ],
            meeting: 'Every other Friday 7:30 PM - 8:30 PM',
            location: 'On our Discord!',
            about: 'Hello there! We’re Rutgers Women’s Gaming League. We’re here to promote equality in gaming and provide a safe and quality environment for gamers of all kinds while uplifting women. We hope to provide the women of Rutgers a comfortable place to express their passion for gaming who are seeking a community where they feel they belong. We value creating a club where everyone feels like they have a space and have people to share their hobbies with, and support all genders in participating in competitive and non competitive gaming. If you’re looking for like-minded friends or just a place to hang out, come check us out! We also hold events and tournaments throughout the semester, be on the lookout for those!',

        },
        {
            name: 'DOTA',
            picture: '/Pictures/Teams/dota.png',
            pictures: [
                '/Pictures/Teams/dota.png',
                '/Pictures/Teams/lol.png',
            ],
            socials: [
                {
                    type: 'facebook',
                    link: 'https://www.facebook.com/groups/rutgersdota/',
                },
                {
                    type: 'discord',
                    link: 'https://discord.gg/yFKghbJ',
                },
            ],
            logo: '/Pictures/Logos/dota2.png',
            people: [
                {
                    name: 'Raj Limbasia',
                    role: 'PRESIDENT',
                },
                {
                    name: 'Eitan Ostrovsky',
                    role: 'VICE PRESIDENT',
                },
                {
                    name: 'Tyler Wang',
                    role: 'TREASURER',
                },
                /* {
                        name: "Kevin Su",
                        role: "Competitive Manager"
                    }, */
            ],
            meeting: 'Friday 8PM-12AM',
            location: 'Science and Engineering Resource Center-Room 206',
            about: 'Welcome to the Dota 2 community page! Whether you’re a casual or competitive player, everyone is welcome at our DotA club! Meetings consist of inhouses (standard 5v5 matches in various modes) to relax and have fun, and we occasionally hold educational days where the more experienced members of the community show tips and tricks to our newer players. Our community is also heavily involved online, whether it be ranked queues on weekday nights or our official inhouse tournament games. If you’re more of the competitive type, we host multiple teams in the Collegiate Star League (CSL). Our club members are always seeking to learn and improve their game and have a great time while doing so.',

        },
        {
            name: 'FIGHTING GAMES',
            picture: '/Pictures/Teams/fgc.png',
            pictures: [
                '/Pictures/Teams/fgc.png',
                '/Pictures/Teams/lol.png',
            ],
            socials: [
                {
                    type: 'facebook',
                    link: 'https://www.facebook.com/groups/199488820593345',
                },
                {
                    type: 'discord',
                    link: 'https://discord.gg/7xqp8WD',
                },
            ],
            logo: '/Pictures/Logos/Sf5logo.png',
            people: [
                {
                    name: 'Victor Okoye',
                    role: 'PRESIDENT',
                },
                {
                    name: 'Jeff Zhang',
                    role: 'VICE PRESIDENT',
                },
                {
                    name: 'Alexander Li',
                    role: 'TREASURER',
                },
                {
                    name: 'Kevin Su',
                    role: 'COMPETITIVE MANAGER',
                },
            ],
            meeting: 'Every other Friday 7:30 PM - 8:30 PM',
            location: 'On our Discord!',
            about: 'Hello there! We’re Rutgers Women’s Gaming League. We’re here to promote equality in gaming and provide a safe and quality environment for gamers of all kinds while uplifting women. We hope to provide the women of Rutgers a comfortable place to express their passion for gaming who are seeking a community where they feel they belong. We value creating a club where everyone feels like they have a space and have people to share their hobbies with, and support all genders in participating in competitive and non competitive gaming. If you’re looking for like-minded friends or just a place to hang out, come check us out! We also hold events and tournaments throughout the semester, be on the lookout for those!',

        },
        {
            name: 'HEARTHSTONE',
            picture: '/Pictures/Teams/hs.png',
            pictures: [
                '/Pictures/Teams/hs.png',
                '/Pictures/Teams/lol.png',
            ],
            socials: [
                {

                    type: 'facebook',
                    link: 'https://www.facebook.com/groups/RUHearthstone',
                },
                {

                    type: 'discord',
                    link: 'https://discord.gg/b7aeQhu',
                },
            ],
            logo: '/Pictures/Logos/hearthstone.png',
            people: [
                {
                    name: 'Victor Okoye',
                    role: 'PRESIDENT',
                },
                {
                    name: 'Jeff Zhang',
                    role: 'VICE PRESIDENT',
                },
                {
                    name: 'Alexander Li',
                    role: 'TREASURER',
                },
                {
                    name: 'Kevin Su',
                    role: 'COMPETITIVE MANAGER',
                },
            ],
            meeting: 'Every other Friday 7:30 PM - 8:30 PM',
            location: 'On our Discord!',
            about: 'Hello there! We’re Rutgers Women’s Gaming League. We’re here to promote equality in gaming and provide a safe and quality environment for gamers of all kinds while uplifting women. We hope to provide the women of Rutgers a comfortable place to express their passion for gaming who are seeking a community where they feel they belong. We value creating a club where everyone feels like they have a space and have people to share their hobbies with, and support all genders in participating in competitive and non competitive gaming. If you’re looking for like-minded friends or just a place to hang out, come check us out! We also hold events and tournaments throughout the semester, be on the lookout for those!',

        },

        {
            name: 'LEAGUE OF LEGENDS',
            picture: '/Pictures/Teams/lol.png',
            pictures: [
                '/Pictures/Teams/lol.png',
                '/Pictures/Teams/lol.png',
            ],
            socials: [
                {
                    type: 'facebook',
                    link: 'https://www.facebook.com/groups/RUleagueoflegends/',
                },
                {
                    type: 'discord',
                    link: 'https://discord.gg/skqkEYD',
                },
            ],
            logo: '/Pictures/Logos/LoLicon.png',
            people: [
                {
                    name: 'Vikram Vijayakumar',
                    role: 'PRESIDENT',
                },
                {
                    name: 'Matthew Vandeneede',
                    role: 'VICE PRESIDENT',
                },
                {
                    name: 'Harmandeep Saini',
                    role: 'TREASURER',
                },
                {
                    name: 'David Prado',
                    role: 'COMPETITIVE MANAGER',
                },
            ],
            meeting: 'Every other Friday 7:30 PM - 8:30 PM',
            location: 'On our Discord!',
            about: 'Hello there! We’re Rutgers Women’s Gaming League. We’re here to promote equality in gaming and provide a safe and quality environment for gamers of all kinds while uplifting women. We hope to provide the women of Rutgers a comfortable place to express their passion for gaming who are seeking a community where they feel they belong. We value creating a club where everyone feels like they have a space and have people to share their hobbies with, and support all genders in participating in competitive and non competitive gaming. If you’re looking for like-minded friends or just a place to hang out, come check us out! We also hold events and tournaments throughout the semester, be on the lookout for those!',

        },
        {
            name: 'OVERWATCH',
            picture: '/Pictures/Teams/overwatch.jpg',
            pictures: [
                '/Pictures/Teams/overwatch.jpg',
                '/Pictures/Teams/overwatch.jpg',
            ],
            socials: [
                {
                    type: 'facebook',
                    link: 'https://www.facebook.com/groups/325763831241701/',
                },
                {
                    type: 'discord',
                    link: 'https://discord.gg/U6P3Dtp',
                },
            ],
            logo: '/Pictures/Logos/OverwatchWhite.png',
            people: [
                {
                    name: 'Noe Duran',
                    role: 'PRESIDENT',
                },
                {
                    name: 'David',
                    role: 'VICE PRESIDENT',
                },
                {
                    name: 'Krishna',
                    role: 'TREASURER',
                },
                /* {
                        name: "Kevin Su",
                        role: "Competitive Manager"
                    }, */
            ],
            meeting: 'Friday 8PM – 12AM',
            location: 'Science and Engineering Resource Center – Room 211',
            about: 'Welcome to ModernStream Overwatch! Here we discuss, share, and collect all things Overwatch. Interested in finding some friends to play with? Want to learn how to aim like a pro? Rutgers Overwatch provides a whole community of players with varying knowledge and skills, giving one of the most unique gaming experiences on campus. Every week we meet and host matches, tournaments, and all-around fun times in SERC 211 on Busch Campus. Come check us out and be apart of an incredible community! We have our own national collegiate team. So, if you think you’ve got the skill to represent the Scarlet Knights against other colleges, come to our meetings and talk to one of our Team Captains!',

        },
        {
            name: 'POKEMON',
            picture: '/Pictures/Teams/pkmn.png',
            pictures: [
                '/Pictures/Teams/pkmn.png',
                '/Pictures/Teams/lol.png',
            ],
            socials: [
                {
                    type: 'facebook',
                    link: 'https://www.facebook.com/groups/RUPokemon/',
                },
                {
                    type: 'discord',
                    link: 'https://discord.gg/WCgKXts',
                },
                {
                    type: 'twitch',
                    link: 'https://www.twitch.tv/rutgerspokemon',
                },
            ],
            logo: '/Pictures/Logos/Pokeball2.png',
            people: [
                {
                    name: 'Victor Okoye',
                    role: 'PRESIDENT',
                },
                {
                    name: 'Jeff Zhang',
                    role: 'VICE PRESIDENT',
                },
                {
                    name: 'Alexander Li',
                    role: 'TREASURER',
                },
                {
                    name: 'Kevin Su',
                    role: 'COMPETITIVE MANAGER',
                },
            ],
            meeting: 'Every other Friday 7:30 PM - 8:30 PM',
            location: 'On our Discord!',
            about: 'Hello there! We’re Rutgers Women’s Gaming League. We’re here to promote equality in gaming and provide a safe and quality environment for gamers of all kinds while uplifting women. We hope to provide the women of Rutgers a comfortable place to express their passion for gaming who are seeking a community where they feel they belong. We value creating a club where everyone feels like they have a space and have people to share their hobbies with, and support all genders in participating in competitive and non competitive gaming. If you’re looking for like-minded friends or just a place to hang out, come check us out! We also hold events and tournaments throughout the semester, be on the lookout for those!',

        },
        {
            name: 'RAINBOW SIX SIEGE',
            picture: '/Pictures/Teams/r6.png',
            pictures: [
                '/Pictures/Teams/r6.png',
                '/Pictures/Teams/lol.png',
            ],
            socials: [
                {
                    type: 'facebook',
                    link: 'https://www.facebook.com/groups/RutgersR6S',
                },
                {
                    type: 'discord',
                    link: ' https://discord.gg/3KuRSFH',
                },
            ],
            logo: '/Pictures/Logos/rainbow6.png',
            people: [
                {
                    name: 'Victor Okoye',
                    role: 'PRESIDENT',
                },
                {
                    name: 'Jeff Zhang',
                    role: 'VICE PRESIDENT',
                },
                {
                    name: 'Alexander Li',
                    role: 'TREASURER',
                },
                {
                    name: 'Kevin Su',
                    role: 'COMPETITIVE MANAGER',
                },
            ],
            meeting: 'Every other Friday 7:30 PM - 8:30 PM',
            location: 'On our Discord!',
            about: 'Hello there! We’re Rutgers Women’s Gaming League. We’re here to promote equality in gaming and provide a safe and quality environment for gamers of all kinds while uplifting women. We hope to provide the women of Rutgers a comfortable place to express their passion for gaming who are seeking a community where they feel they belong. We value creating a club where everyone feels like they have a space and have people to share their hobbies with, and support all genders in participating in competitive and non competitive gaming. If you’re looking for like-minded friends or just a place to hang out, come check us out! We also hold events and tournaments throughout the semester, be on the lookout for those!',

        },

        {
            name: 'RHYTHM GAMES',
            picture: '/Pictures/Teams/rgc.png',
            pictures: [
                '/Pictures/Teams/rgc.png',
                '/Pictures/Teams/lol.png',
            ],
            socials: [
                {
                    type: 'facebook',
                    link: 'https://www.facebook.com/groups/rutgersrgc/',
                },
                {
                    type: 'discord',
                    link: 'https://discord.gg/azP28Hd',
                },
            ],
            logo: '/Pictures/Logos/RGCLOGO2.png',
            people: [
                {
                    name: 'Victor Okoye',
                    role: 'PRESIDENT',
                },
                {
                    name: 'Jeff Zhang',
                    role: 'VICE PRESIDENT',
                },
                {
                    name: 'Alexander Li',
                    role: 'TREASURER',
                },
                {
                    name: 'Kevin Su',
                    role: 'COMPETITIVE MANAGER',
                },
                /*  {
                        name: "Kevin Su",
                        role: "Competitive Manager"
                    }, */
            ],
            meeting: 'Friday 8PM – 12AM',
            location: 'Science and Engineering Resource Center – Room 218',
            about: 'The ModernStream Rhythm Game Community is a group of people that love all sorts of music games wanting to feel the rhythm. We have players for osu!, Stepmania, DDR/ITG, GH/RB, Bemani titles, and more. Whether you are a pro or someone starting out, everyone is welcome.',

        },
        {
            name: 'ROCKET LEAGUE',
            picture: '/Pictures/Teams/rl.png',
            pictures: [
                '/Pictures/Teams/rl.png',
                '/Pictures/Teams/lol.png',
            ],
            socials: [
                {
                    type: 'facebook',
                    link: 'https://www.facebook.com/groups/128519717832825/',
                },
                {
                    type: 'discord',
                    link: 'https://discord.gg/nuSzTWw',
                },
            ],
            logo: '/Pictures/Logos/RocketLeague.png',
            people: [
                {
                    name: 'Victor Okoye',
                    role: 'PRESIDENT',
                },
                {
                    name: 'Jeff Zhang',
                    role: 'VICE PRESIDENT',
                },
                {
                    name: 'Alexander Li',
                    role: 'TREASURER',
                },
                {
                    name: 'Kevin Su',
                    role: 'COMPETITIVE MANAGER',
                },
            ],
            meeting: 'Friday 8PM – 12AM',
            location: 'Science and Engineering Resource Center – Room 212',
            about: 'Hello there! We’re Rutgers Women’s Gaming League. We’re here to promote equality in gaming and provide a safe and quality environment for gamers of all kinds while uplifting women. We hope to provide the women of Rutgers a comfortable place to express their passion for gaming who are seeking a community where they feel they belong. We value creating a club where everyone feels like they have a space and have people to share their hobbies with, and support all genders in participating in competitive and non competitive gaming. If you’re looking for like-minded friends or just a place to hang out, come check us out! We also hold events and tournaments throughout the semester, be on the lookout for those!',

        },
        {
            name: 'SMASH MELEE',
            picture: '/Pictures/Teams/melee.png',
            pictures: [
                '/Pictures/Teams/melee.png',
                '/Pictures/Teams/lol.png',
            ],
            socials: [
                {
                    type: 'facebook',
                    link: 'https://www.facebook.com/groups/scarletsmash/',
                },
                {
                    type: 'discord',
                    link: 'https://discord.gg/SEDm5MM',
                },
            ],
            logo: '/Pictures/Logos/smashMelee.png',
            people: [
                {
                    name: 'Victor Okoye',
                    role: 'PRESIDENT',
                },
                {
                    name: 'Jeff Zhang',
                    role: 'VICE PRESIDENT',
                },
                {
                    name: 'Alexander Li',
                    role: 'TREASURER',
                },
                {
                    name: 'Kevin Su',
                    role: 'COMPETITIVE MANAGER',
                },
            ],
            meeting: 'Every other Friday 7:30 PM - 8:30 PM',
            location: 'On our Discord!',
            about: 'Hello there! We’re Rutgers Women’s Gaming League. We’re here to promote equality in gaming and provide a safe and quality environment for gamers of all kinds while uplifting women. We hope to provide the women of Rutgers a comfortable place to express their passion for gaming who are seeking a community where they feel they belong. We value creating a club where everyone feels like they have a space and have people to share their hobbies with, and support all genders in participating in competitive and non competitive gaming. If you’re looking for like-minded friends or just a place to hang out, come check us out! We also hold events and tournaments throughout the semester, be on the lookout for those!',

        },
        {
            name: 'SMASH ULTIMATE',
            pictures: [
                '/Pictures/Teams/ultimate.png',
                '/Pictures/Teams/lol.png',
            ],
            socials: [
                {
                    type: 'facebook',
                    link: 'https://www.facebook.com/groups/scarletsmash/',
                },
                {
                    type: 'twitter',
                    link: 'https://twitter.com/scarletsmash',
                },
                {
                    type: 'discord',
                    link: 'https://discord.gg/8BcAwgk',
                },

            ],
            logo: '/Pictures/Logos/SmashUltimateWhite.png',
            people: [
                {
                    name: 'Alex Wu',
                    role: 'PRESIDENT',
                },
                {
                    name: 'Ryan Carey',
                    role: 'VICE PRESIDENT',
                },
                {
                    name: 'Matthew Henry',
                    role: 'TREASURE',
                },

            ],
            meeting: 'Friday 8PM - 12AM',
            location: 'Science and Engineering Resource Center - Room 203',
            about: 'This part of Scarlet Smash is for Smash Ultimate enthusiasts of all skill levels to socialize, play friendlies, or test their skill in our free biweekly tournaments. We meet every Friday from 8-12 pm at SERC with a bracket every other week, and fun casual events on other weeks!',

        },
        {
            name: 'RU Valowatch',
            pictures: [
                '/Pictures/Teams/overwatch.jpg',
                '/Pictures/Teams/overwatch.jpg',
            ],
            socials: [
                {
                    type: 'twitter',
                    link: 'https://twitter.com/Valowatch',
                },
                {
                    type: 'twitch',
                    link: 'https://www.twitch.tv/ruvalowatch',
                },
                {
                    type: 'discord',
                    link: 'https://discord.gg/U6P3Dtp',
                },

            ],
            logo: '/Pictures/Logos/ValorantDiscordAlt.png',
            people: [
                {
                    name: 'Noé Duran',
                    role: 'PRESIDENT',
                },
                {
                    name: 'David Sotimehin',
                    role: 'VICE PRESIDENT',
                },
                {
                    name: 'Krishna Rayini',
                    role: 'TREASURE',
                },

            ],
            meeting: 'Friday 6PM - 7PM start time',
            location: 'On our Discord',
            about: "ModernStream VALORANT and Overwatch club, or RU Valowatch, is a club meant for both casual and competitive players to interact and enjoy playing VALORANT and Overwatch together. Between bi-weekly in-houses for both games, a multitude of events with various prizes for both our best participants and our most enthusiastic, or even just hanging out and getting to learn from our respective competitive teams, Valowatch club has a lot to offer for anyone interested in either VALORANT or Overwatch - and if there happens to be any other games that suit your fancy, there's bound to be a community here for that too.",

        },
        {
            name: "WOMEN'S GAMING LEAGUE",
            pictures: [
                '/Pictures/Teams/ultimate.png',
                '/Pictures/Teams/lol.png',
            ],
            socials: [
                {
                    type: 'twitter',
                    link: 'https://twitter.com/ModernStreamsWGL',
                },
                {
                    type: 'twitch',
                    link: 'https://www.twitch.tv/rutgerswgl',
                },
                {
                    type: 'instagram',
                    link: 'https://www.instagram.com/rutgerswgl/',
                },
                {
                    type: 'mailchimp',
                    link: 'https://mailchi.mp/4a7e164ed9b8/subscribe-to-the-rwgl-newsletter',
                },
                {
                    type: 'discord',
                    link: 'https://discord.com/invite/GgHVaYt',
                },

            ],
            logo: '/Pictures/Logos/WomenGamingLeague.png',
            people: [
                {
                    name: 'Ashley Stankovits',
                    role: 'PRESIDENT',
                },
                {
                    name: 'Cara Escalona',
                    role: 'PRESIDENT',
                },
                {
                    name: 'Kristen Marcinek',
                    role: 'VICE PRESIDENT',
                },
                {
                    name: 'Rae Hochwald',
                    role: 'TREASURER',
                },
                {
                    name: 'Autumn Rauscher',
                    role: 'SECRETARY',
                },
            ],
            meeting: 'Every other Friday 7:30 PM - 8:30 PM',
            location: 'On our Discord!',
            about: 'Hello there! We’re Rutgers Women’s Gaming League. We’re here to promote equality in gaming and provide a safe and quality environment for gamers of all kinds while uplifting women. We hope to provide the women of Rutgers a comfortable place to express their passion for gaming who are seeking a community where they feel they belong. We value creating a club where everyone feels like they have a space and have people to share their hobbies with, and support all genders in participating in competitive and non competitive gaming. If you’re looking for like-minded friends or just a place to hang out, come check us out! We also hold events and tournaments throughout the semester, be on the lookout for those!',
        },

    ],

};
